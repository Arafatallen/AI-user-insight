...
// 获取用户自己安装的 skills（仅 ~/.openclaw/skills/，排除系统自带）
function getUserInstalledSkills(home) {
  ...
  return names.filter(n => n && !n.startsWith('.')).map(name => {
    ...
    return { name, description, source: 'user-installed' };
  });
}

// 获取系统内置 skills（openclaw/skills/ 目录下的）
function getBuiltInSkills(home) {
  const builtInDirs = [
  ...
  for (const dir of builtInDirs) {
    ...
    for (const name of list) {
      if (name && !seen.has(name) && !name.startsWith('.') && !name.includes('.zip') && !name.includes('.skill')) {
        seen.add(name);
        skills.push({ name, source: 'built-in' });
      }
    }
  }
  return skills;
...
// 获取所有可用 skills（用于展示，但分析时区分来源）
function getAllAvailableSkills(home) {
  const userSkills = getUserInstalledSkills(home);
  const builtInSkills = getBuiltInSkills(home);
  
  return {
    userInstalled: userSkills,
    builtIn: builtInSkills,
    all: [...userSkills, ...builtInSkills.filter(b => !userSkills.some(u => u.name === b.name))]
  };
}
...
function collect({ workspace, projects, home }) {
  ...
  // 技能统计（关键区分：用户安装 vs 系统内置）
  const allSkills = getAllAvailableSkills(home);
  const userInstalledSkills = allSkills.userInstalled;  // 只统计用户自己安装的
  const builtInSkills = allSkills.builtIn;              // 系统自带的
  const projectsList = getProjects(projects);
  const agentModel = getAgentModel(home);

  // 判定囤积指数（只看用户安装的，不看系统自带的）
  const hoardingLevel = userInstalledSkills.length <= 2 ? 'minimal' : 
                        userInstalledSkills.length <= 5 ? 'moderate' : 'high';
  ...
  // 技能分类（基于用户实际使用的 skills，从 memory 提取）
  const skillCategories = {
    coding: skillFrequency.topSkills.filter(s => /coding|github|git|dev|api|crawler|scrap|agent/i.test(s.name)).length,
    content: skillFrequency.topSkills.filter(s => /video|image|audio|tts|blog|news|stock|interview|remotion/i.test(s.name)).length,
    data: skillFrequency.topSkills.filter(s => /analysis|expert|aggregator|pdf|search|insight/i.test(s.name)).length,
    social: skillFrequency.topSkills.filter(s => /discord|slack|telegram|whatsapp|imsg/i.test(s.name)).length,
    finance: skillFrequency.topSkills.filter(s => /stock|investment|buffett|finance/i.test(s.name)).length,
    productivity: skillFrequency.topSkills.filter(s => /feishu|notion|todo|calendar|email|reminder/i.test(s.name)).length,
  };

  // 判定职业方向（基于实际使用的 skill 类型）
  const dominantCategory = Object.entries(skillCategories)
    .sort((a, b) => b[1] - a[1])[0];
  const careerDirection = dominantCategory[0];  // coding/content/data/social/finance/productivity
  ...
  const result = {
    ...
    // 核心统计（关键区分）
    stats: {
      userInstalledSkills: userInstalledSkills.length,      // 用户自己安装的（判定囤积）
      builtInSkills: builtInSkills.length,                // 系统自带的
      totalAvailableSkills: allSkills.all.length,         // 总共可用
      ...
      hoardingLevel,                                      // 极简/适度/重度
      careerDirection,                                   // coding/content/data/social/finance/productivity
    },
    ...
    recent: {
      ...
      userInstalledSkills: userInstalledSkills.map(s => ({ name: s.name, desc: s.description })),  // 只展示用户安装的
    },
